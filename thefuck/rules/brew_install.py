...
def match(command):
    is_proper_command = (
        "brew install" in command.script and "No available formula" in command.output
    )

    if is_proper_command:
        formula = re.findall(
            r"Error: No available formula for ([a-z]+)", command.output
        )[0]
