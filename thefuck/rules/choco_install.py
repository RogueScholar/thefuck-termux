...
@for_app("choco", "cinst")
def match(command):
    return (command.script.startswith("choco install")
            or "cinst" in command.script_parts
...
def get_new_command(command):
    ...
    for script_part in command.script_parts:
        if (script_part not in ["choco", "cinst", "install"]
            ...
            return command.script.replace(script_part,
                                          script_part + ".install")
    return []
