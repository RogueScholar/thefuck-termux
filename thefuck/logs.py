...
def show_corrected_command(corrected_command):
    sys.stderr.write(u"{prefix}{bold}{script}{reset}{side_effect}\n".format(
        ...
        if corrected_command.side_effect else u"",
...
def confirm_text(corrected_command):
    sys.stderr.write((u"{prefix}{clear}{bold}{script}{reset}{side_effect} "
                      ...
                      u"/{red}ctrl+c{reset}]").format(
                          ...
                          if corrected_command.side_effect else "",
                          clear="\033[1K\r",
                          bold=color(colorama.Style.BRIGHT),
...
def how_to_configure_alias(configuration_details):
    ...
    if configuration_details:
        print(u"Please put {bold}{content}{reset} in your "
        ...
        if configuration_details.can_configure_automatically:
            print(u"Or run {bold}fuck{reset} a second time to configure"
...
def configured_successfully(configuration_details):
    print(u"{bold}fuck{reset} alias configured successfully!\n"
          u"For applying changes run {bold}{reload}{reset}"
          u" or restart your shell.".format(
              bold=color(colorama.Style.BRIGHT),
              reset=color(colorama.Style.RESET_ALL),
